...
number = int(input())
image = Image.open('/Users/hy_mbp/PycharmProjects/Py_lesson/test.png')
width, gao = image.size
item_width = width//number
item_gao = gao//number
image.resize([(item_gao*number),(item_width*number)])
box_list = []
for i in range(0,number):
    for j in range(0,number):
        box = (j*item_width,i*item_gao,(j+1)*item_width,(i+1)*item_gao)
        box_list.append(box)
image_list = []
for box in box_list:
