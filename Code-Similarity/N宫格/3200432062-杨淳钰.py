...
count = int(sqrt(int(input("请输入平方数："))))
image = Image.open("nit.jpg")
W, H = image.size

image = image.crop((0, 0, W - W % count, H - H % count))#裁剪class 'PIL.Image.Image'
W, H = image.size
...
for h in range(0, H, height):
    for w in range(0, W, width):
        ...
        index = index + 1
temp = []
random.shuffle(array)#随机排列
for i in range(0, len(array), count):
    temp.append(np.concatenate(tuple(array[i:i+count]), axis=1))#并列
